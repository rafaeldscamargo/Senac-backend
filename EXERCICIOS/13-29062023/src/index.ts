...
function calculateDebits () {
    return users.map(user => {
        const sumDebits = user.debit.reduce((acumulator: number, value: number) =>
         //acumulator = acumulator + value === acumulator += value
       value ? acumulator += value : 0
        , 0);
        
        //user.totalBalance -= sumDebits === user.totalBalance = user.totalBalance - sumDebits
